    ...
    case 'POST':
      if (xCallbackToken !== callbackToken) {
        ...
        try {
          ...
          await contract.storePG(resBody.id, resBody.external_id, resBody.payment_method, resBody.status, resBody.amount, payment, resBody.paid_at, resBody.payer_email, resBody.description, resBody.currency, resBody.payment_channel)
          ...
          fs.readFile('./public/template/invoice.ejs', 'utf8', function (err, content) {
            ...
            pdf
              ...
              .toStream(async function (err, stream) {
                ...
                const writeStream = fs.createWriteStream(`./public/pdf/${resBody.external_id}.pdf`);
                stream.pipe(writeStream);
                writeStream.on('finish', async () => {
                  try {
                    const file = await fs.promises.readFile(`./public/pdf/${resBody.external_id}.pdf`);
                    const fileAdded = await ipfs.add(file);
                    console.log(fileAdded);
                      await contract.updateIPFS(resBody.external_id, fileAdded.path);
                  } catch (err) {
